...
import {
  ...
  DialogTitle, IconButton, AppBar, CircularProgress, Toolbar,
      ...
      <AppBar color={"primary"} enableColorOnDark>
        <Toolbar>
          <table width={'100%'}>
            <tr>
              <td width={'20px'}>
                <IconButton onClick={() => { setMenuOpen(true) }} style={{ marginRight: "15px" }}>
                  <MenuIcon />
                </IconButton>
              </td>
              <td width={'70px'}>
                <img src={`ct/widecenter${(props.DarkMode ? 'white' : '')}.png`} alt="ClassTrack Logo" height="40" style={{ marginTop: '7px' }} />
              </td>
              <td>
                <Search>
                  <SearchIconWrapper>
              ...
              {/* This will probably need to be switched to an Autocomplete */}
              {
                props.Session
                  ? <> {props.User
                    ? <>
                      <td width={'200px'}>
                        <WelcomeLabel user={props.User} DarkMode={props.DarkMode} />
                      </td>
                      <td width={'20px'}>
                        <SettingsButton User={props.User} Session={props.Session} DarkMode={props.DarkMode} RefreshUser={props.RefreshUser} /></td>
                      <td width={'20px'}>
